function calcShipping(sum, min, shipping) {
	let productsSum = sum; // сумма в корзине <== (не хватаетало буквы "s")
	...
	if (productsSum == 0) {												// если productsSum равно 0
		shippingPrice = 0;												// то shippingPrice присвоить значение 0
	}
	else if (productsSum >= freeShippingMinSum) {			// если productsSum > или равен freeShippingMinSum,
		shippingPrice = 0;											// то shippingPrice присвоить значение 0
	}
...
function calcDiscount(sum, min, discount) {
	...
	let discountSum = (productsSum >= discountMinSum) ? (productsSum / 100) * discountPart : 0;		// создайте переменную discountPrice <== Неправильное название переменной
